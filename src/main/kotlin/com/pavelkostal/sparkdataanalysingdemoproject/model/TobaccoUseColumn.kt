...
enum class TobaccoUseColumn(val columnName: String, val description: String, val apiFieldName: String, val dataType: DataType) {
    ...
    DATA_VALUE("Data_Value", "Value of the data", "data_value", DataTypes.StringType),
    ...
    DATA_VALUE_STD_ERR("Data_Value_Std_Err", "Standard error for the data value", "data_value_std_err", DataTypes.StringType),
    LOW_CONFIDENCE_LIMIT("Low_Confidence_Limit", "Confidence interval lower limit", "low_confidence_limit", DataTypes.StringType),
    HIGH_CONFIDENCE_LIMIT("High_Confidence_Limit", "Confidence interval upper limit", "high_confidence_limit", DataTypes.StringType),
    SAMPLE_SIZE("Sample_Size", "Sample size", "sample_size", DataTypes.StringType),
